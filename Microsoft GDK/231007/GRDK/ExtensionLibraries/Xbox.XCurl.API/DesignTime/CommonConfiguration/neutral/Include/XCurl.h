...
#define CURL_MAX_WRITE_SIZE 8192
#endif
...
/* curl_strequal() and curl_strnequal() are subject for removal in a future
   release */
#define curl_strequal strcmp
#define curl_strnequal strncmp
...
#ifdef XCURL_INCLUDE_UNSUPPORTED_API
/*
...
typedef
void*
(CALLBACK* xcurl_alloc_memory_callback)(
    size_t size,
    uint32_t memoryType
    );
...
typedef
void
(CALLBACK* xcurl_free_memory_callback)(
    void* ptr,
    uint32_t memoryType
    );
...
typedef
void*
(CALLBACK* xcurl_realloc_memory_callback)(
    void* ptr,
    size_t size,
    uint32_t memoryType
    );
...
typedef
void*
(CALLBACK* xcurl_calloc_memory_callback)(
    size_t nmemb,
    size_t size,
    uint32_t memoryType
    );
...
CURL_EXTERN void xcurl_global_suspend();

CURL_EXTERN void xcurl_global_resume();

/*
 * xcurl_global_set_request_limit will cause XCurl to automatically limit the
 * number of active requests and serialize any additional requests beyond this
 * limit.  You may still create as many Easy handles as you would like,
 * configure them, and add them to multi handles or call curl_easy_perform on
 * them regardless of this limit.
 * The limit defaults to 12.  You may set this limit at any point, including
 * before initializing XCurl, and changing this value will not impact
 * in-progress requests.
 */
CURL_EXTERN void xcurl_global_set_request_limit(
    uint32_t limit
    );
